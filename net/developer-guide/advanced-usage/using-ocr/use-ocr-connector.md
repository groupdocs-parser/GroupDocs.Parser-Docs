...
Parser parser = new Parser(fileName, settings);
```
...
public class YourCustomOcrConnector : OcrConnectorBase
{
    public override bool IsTextSupported => true;

    public override bool IsTextAreasSupported => true;

    public override string RecognizeText(Stream imageStream, OcrOptions options)
    {
        // Implement OCR logic here
        return string.Empty;
    }

    public override IList<PageTextArea> RecognizeTextAreas(Stream imageStream, Page page, OcrOptions options)
    {
        // Implement OCR for text areas here
        return new List<PageTextArea>();
    }

    public override IList<PageTextArea> RecognizeTextAreas(
        Stream imageStream,
        ...
        OcrOptions options)
    {
        // Implement OCR for specific rectangles here
        return new List<PageTextArea>();
    }
}
```
...
If this property returns `true`, then you need to implement the `RecognizeTextAreas` methods. In addition to the image and OCR settings, these methods receive the document's [Page](https://reference.groupdocs.com/parser/net/groupdocs.parser.data/page) class and, in a more advanced version, a list of rectangular areas to recognize and a list of allowed characters.
